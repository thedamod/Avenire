...
export const config = {
  ...
  cohereApiKey: normalizeSecret(
    process.env.COHERE_API_KEY ?? process.env.VOYAGE_API_KEY
  ),
  cohereEmbedModel: process.env.COHERE_EMBED_MODEL ?? "embed-v4.0",
  cohereRpmLimit: Number.parseInt(
    process.env.COHERE_RPM_LIMIT ?? process.env.VOYAGE_RPM_LIMIT ?? "12",
    10
  ),
  cohereTpmLimit: Number.parseInt(
    process.env.COHERE_TPM_LIMIT ?? process.env.VOYAGE_TPM_LIMIT ?? "200000",
    10
  ),
  cohereImageTokenEstimate: Number.parseInt(
    process.env.COHERE_IMAGE_TOKEN_ESTIMATE ??
      process.env.VOYAGE_IMAGE_TOKEN_ESTIMATE ??
      "1200",
    10
  ),
  cohereRetryMaxAttempts: Number.parseInt(
    process.env.COHERE_RETRY_MAX_ATTEMPTS ??
      process.env.VOYAGE_RETRY_MAX_ATTEMPTS ??
      "4",
    10
  ),
  cohereRetryBaseDelayMs: Number.parseInt(
    process.env.COHERE_RETRY_BASE_DELAY_MS ??
      process.env.VOYAGE_RETRY_BASE_DELAY_MS ??
      "1200",
    10
  ),
  cohereTestSafeMode:
  ...
  ingestionStageTimingLog:
    ...
    "false",
  linkExtractionProvider: (process.env.LINK_EXTRACTION_PROVIDER ?? "tavily") as
    | "tavily"
    | "firecrawl",
