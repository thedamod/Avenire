...
// Run: python3 packages/ai/generative-ui/scripts/generate_guidelines_ts.py

const SECTION_BY_LABEL: Record<string, string> = {
  "_preamble": `# Imagine — Visual Creation Suite`,
  "Modules": `## Modules
...
const MODULE_MAPPING: Record<string, string[]> = {
  "art": ["_preamble", "Modules", "Core Design System", "When nothing fits", "SVG setup", "Art and illustration"],
  "mockup": ["_preamble", "Modules", "Core Design System", "When nothing fits", "UI components", "Color palette"],
  "interactive": ["_preamble", "Modules", "Core Design System", "When nothing fits", "UI components", "Color palette"],
  "chart": ["_preamble", "Modules", "Core Design System", "When nothing fits", "UI components", "Color palette", "Charts (Chart.js)"],
  "diagram": ["_preamble", "Modules", "Core Design System", "When nothing fits", "Color palette", "SVG setup", "Diagram types"],
  "physics": ["_preamble", "Modules", "Core Design System", "When nothing fits", "Physics Simulation Module"],
};

export const AVAILABLE_MODULES = Object.keys(MODULE_MAPPING);
...
export function getGuidelines(modules: string[]): string {
  ...
  for (const mod of modules) {
    const sections = MODULE_MAPPING[mod];
    ...
    for (const label of sections) {
      ...
      const sectionText = SECTION_BY_LABEL[label];
      if (sectionText === undefined) {
        throw new Error(`Unknown guideline section label: ${label}`);
      }
