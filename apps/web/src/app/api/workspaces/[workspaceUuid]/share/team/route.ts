...
/**
 * Share a workspace with its organization team: grant necessary chat access and notify members by email.
 *
 * Validates the caller's session and admin role for the workspace, grants chat permissions to eligible team members,
 * sends workspace share emails to those members, records metrics and logging, and returns sharing results.
 *
 * @param context - Request context containing `params.workspaceUuid`, the workspace identifier to share
 * @returns An object with `recipients` (number of team members considered), `emailSentCount` (number of successfully sent emails), and `workspaceUrl` (the files view URL used in the share emails)
 */
